...
async def ensure_initialized():
    ...
    if "default" not in websockets:
        logger.info("Lazy-initializing default Python context...")
        try:
            python_context = await create_context(
                client, websockets, "python", "/home/user"
            )
            default_websockets["python"] = python_context.id
            websockets["default"] = websockets[python_context.id]
            logger.info(f"Default context initialized: {python_context.id}")
        except Exception as e:
            logger.error(f"Failed to lazy-init context: {e}")
            raise e
